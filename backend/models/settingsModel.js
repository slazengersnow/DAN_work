...
const settingsModel = {
  ...
  updateSettings: async (settingsData) => {
    try {
      // 設定が存在するか確認
      const existingSettings = await db.query('SELECT 1 FROM settings WHERE id = 1');
  ...
  deleteDepartment: async (id) => {
    try {
      ...
      if (employeeCheck.rows.length > 0) {
        throw new Error('この部門に所属する従業員が存在するため削除できません');
      }
  ...
  getEmploymentStatuses: async () => {
    try {
      const result = await db.query('SELECT * FROM employment_statuses ORDER BY name');
      return result.rows;
    } catch (error) {
      throw error;
    }
  ...
  deleteEmploymentStatus: async (id) => {
    try {
      ...
      if (employeeCheck.rows.length > 0) {
        throw new Error('この雇用形態の従業員が存在するため削除できません');
      }
  ...
  getDisabilityTypes: async () => {
    try {
      const result = await db.query('SELECT * FROM disability_types ORDER BY name');
      return result.rows;
    } catch (error) {
      throw error;
    }
  ...
  deleteDisabilityType: async (id) => {
    try {
      ...
      const disabilityCheck = await db.query(
      ...
      if (disabilityCheck.rows.length > 0) {
        throw new Error('この障害種別の従業員が存在するため削除できません');
      }
